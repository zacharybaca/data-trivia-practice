...
function listAllCitiesWithCountries(climateData) {
    // Your code here
    return climateData.map(ele => `${ele.city}, ${ele.country}`)
}
...
function listAllUsCities(climateData) {
    // Your code here
    let newArr = [];
    climateData.filter(ele => ele.country === "United States" ? newArr.push(`${ele.city}, ${ele.country}`) : null)
    return newArr;
}
...
function findDuplicates(climateData) {
    ...
    for (let i = 0; i < climateData.length; i++) {
        if (!Object.hasOwn(dupObj, climateData[i].city)) {
            dupObj[climateData[i].city] = [climateData[i].id]
        } else {
            dupObj[climateData[i].city].push(climateData[i].id)
        }
    }
    return dupObj;
}
...
function returnDuplicate(climateData) {
    // Your code here
    let data = findDuplicates(climateData);
    let values = Object.values(data);

    for (let i = 0; i < values.length; i++) {
        if (values[i].length > 1) {
            let lastElement = values[i][values[i].length - 1]
            return climateData[lastElement - 1];
        }
    }
}
...
function correctDuplicate(climateData, newCityName) {
    ...
    let id = Object.values(duplicateCity)[0];

    for (let i = 0; i < climateData.length; i++) {
        if (climateData[i].id === id) {
            climateData[i].city = newCityName;
